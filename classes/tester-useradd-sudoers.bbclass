...
# This is done by creating a new /etc/sudoers.d/99-tester.conf
...
set_sudoers () {
	...
	while test "x$setting" != "x"; do
		eval "$PSEUDO echo \"$setting\" >> \"${IMAGE_ROOTFS}\"/etc/sudoers.d/99-tester"
		setting=`echo $remaining | cut -d ';' -f1`
		remaining=`echo $remaining | cut -d ';' -f2-`
	done
	if [ -f "${IMAGE_ROOTFS}/etc/sudoers.d/99-tester" ]; then
		eval "$PSEUDO chmod 0440 \"${IMAGE_ROOTFS}\"/etc/sudoers.d/99-tester"
	fi
}
